...
function renderShell(){
  $('app').innerHTML = `
  <aside class="sidebar">
    <div class="logo"><div class="logo-icon">⚽</div><div class="logo-text"><h2>PENCA 2026</h2><p>Firebase institucional</p></div></div>
    <nav class="menu">
...
function renderRanking(){
  ...
  $('ranking').innerHTML = `<h1>Ranking</h1><div class="underline"></div><p class="subtitle">El admin no participa del ranking.</p><div class="ranking-container"><div class="podium">${podium.map(r=>`<article class="podium-card ${cls[r.position]||''}"><div class="podium-medal">${med[r.position]||'🏅'}</div><h3 class="podium-username">@${esc(r.username)}</h3><div class="podium-points">${r.points} pts</div><div class="podium-correct">${r.aciertos} aciertos</div><button class="details-btn podium-details-btn" onclick="toggleRankingDetails(this)">+</button><div class="ranking-details podium-details">${detailPills(r)}</div></article>`).join('')}</div><div class="ranking-list-header"><div><h3>Resto del ranking</h3><p>Posiciones a partir del 4.º lugar</p></div></div><div class="ranking-list">${rest.map(r=>`<div class="ranking-item"><div class="ranking-row"><span class="position">${r.position}.º</span><span class="ranking-user"><strong>@${esc(r.username)}</strong></span><span>${r.aciertos} aciertos</span><span class="points">${r.points} pts</span><button class="details-btn" onclick="toggleRankingDetails(this)">+</button></div><div class="ranking-details">${detailPills(r)}</div></div>`).join('')}</div></div>`;
}
